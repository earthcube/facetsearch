    ...
    <div class="media-body">
      <b-container fluid="md" class="mt-3">
        <b-row>
          ...
          <b-col
            md="3"
            class="sidebar scrollable text-center green flex-grow-1 flex-shrink-0 overflow-auto"
          >
            <div v-for="facetSetting in facets" :key="facetSetting.title">
              <div class="filter_card">
            ...
            <div class="mb-2 mb-lg-0">
              <header>
                <div v-if="$data.collectionTitle.length">
                  <h1 class="article-title mx-auto">
                    {{ $data.collectionTitle }} Collection
                  </h1>
                </div>
              </header>
              <sendCollectionToNotebook
                v-if="collectionTitle.length > 0"
                :collection-name="collectionTitle"
              ></sendCollectionToNotebook>
            </div>
            <div class="mt-3">
              <div v-for="type in types" :key="type.row" :type="type">
                <!--              {{type}}-->
                ...
                <!--              {{$data.currentClick }}-->
                <div v-for="item in type.content" :key="item.row" :item="item">
                  <CollectionCard
                    :item="item"
            ...
            if (self.FacetsConfig.COLLECTION_FACETS[i].field == "unassigned") {
              self.facets[i] = {
                ...
                items: [
                  {
                    id: "data",
                    count: data_collections.length,
                    isActive: false,
                    name: "data",
                  },
                  {
                    id: "query",
                    count: query_collections.length,
                    isActive: false,
                    name: "query",
                  },
    ...
    populate(name) {
      ...
      localforage
        .iterate(function (value, key) {
          ...
          if (value.type === name) data.push(value);
          // Vue.set(self.collections, self.collections.length, value)
        })
        .then(function () {
          console.log(data);
