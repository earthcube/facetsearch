<template>
  <b-button
    ...
    @click.stop="getCollectionJson(collectionName)"
...
export default {
  ...
  methods: {
    ...
    async getCollectionJson(collName) {
      ...
      console.log(coll);
      if (
        coll.datasets?.length == 0 &&
        coll.queries?.length == 0 &&
        coll.tools?.length == 0
      ) {
        this.obscurePage = false;
        this.$bvToast.toast(
          `Cannot send to a notebook. Please add a dataset, query and/or tool to the collection: `,
          {
            title: "No Objects in Collection",
