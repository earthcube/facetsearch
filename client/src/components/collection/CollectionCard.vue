<template>
  <b-card tag="article" class="rounded-0">
    <b-card-title class="name" v-html="item.value.name"> </b-card-title>
    <b-card-title
      v-if="item.value.pubname"
      class="publisher"
      v-html="item.value.pubname"
    ></b-card-title>
            ...
            <div v-for="facetSetting in facets" :key="facetSetting.title">
              ...
              <div v-if="facetSetting.title == 'All Collections'">
                <v-select
                  ...
                  @update:model-value="
                    (name) => updateItemCollections(item, name)
                  "
                ></v-select>
              </div>
            </div>
    ...
    <div v-if="item.collection == 'assigned'">
      <div v-if="currentClick == 'assigned'">
        <b-container fluid="md" class="mt-3">
          <b-row> Has been in Collections: </b-row>
          <b-row>
            <b-col cols="8">
              <div v-for="facetSetting in facets" :key="facetSetting.title">
                <!--                                                  {{item}}-->
                <div v-if="facetSetting.title == 'All Collections'">
                  <v-select
                    ...
                    @update:model-value="
                      (name) => updateItemCollections(item, name, false)
                    "
                  ></v-select>
                </div>
              </div>
              ...
              <div v-for="facetSetting in facets" :key="facetSetting.title">
                ...
                <div v-if="facetSetting.title == 'All Collections'">
                  <v-select
                    ...
                    :options="facetSetting.names"
                    @option:selected="
                      (event) => updateItemCollections(item, event)
                    "
                    @option:deselected="(event) => optionRemoved(event, item)"
                  ></v-select>
                </div>
  ...
  methods: {
    ...
    updateRemovedCollection: function (item, removeCollectionNames, isRemove) {
      ...
      for (let i = 0; i < removeCollectionNames.length; i++) {
        ...
        if (isRemove && "assignedCollections" in item)
          if (itemCollections.includes(removeCollectionNames[i])) {
            ...
            itemCollections = itemCollections.filter(
              (name) => removeCollectionNames[i] != name
            );
          }
      }
      ...
      if (!isRemove) {
        ...
        console.log(
          "updateRemovedCollection: remove from coll: " + removeCollectionNames
        );
        this.moveToCollectionStorage(item);
    ...
    async removeFromCollection(item) {
      if (item.removeCollection.length === 0) return;
      var self = this;
      ...
      if (ok) {
        ...
        localforage.getItem(item.value.g, function (err, value) {
          ...
          localforage
            .setItem(item.value.g, toRaw(item))
            .then(() => {
    ...
    populateAssignedCollection: function (type, collname) {
      ...
      localforage
        .iterate(function (value, key) {
          ...
          if (
            value.type === type &&
            (value.assignedCollections === collname ||
              (Array.isArray(value.assignedCollections) &&
                value.assignedCollections.includes(collname)))
          )
            colls.push(value);
        ...
        .then(function () {
          ...
          var selectedcoll = self.selectedCollectionItems[collname].filter(
            (item) => item.type === type
          );
          for (
            let i = 0;
            i <
            self.selectedCollectionItems[self.selectedCollectionName].length;
            i++
          ) {
            var item =
              self.selectedCollectionItems[self.selectedCollectionName][i];
            if (type === item.type) {
