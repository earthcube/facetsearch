<template>
  <div class="col-8">
    <div class="row">
      <span class="font-weight-bold font-heavy my-4" v-html="mapping.s_name">
      </span>
    </div>

    <b-card no-body>
      <b-tabs id="myTabContent" card>
        <b-tab id="md" title="Metadata" active aria-labelledby="md-tab">
          <div class="row">
            <span class="col-2 font-weight-bold">Type:</span>
            <span class="col-8">
              <b-badge v-for="t in mapping.types" :key="t">
                {{ t }}
              </b-badge>
            </span>
          </div>
          <div class="row">
            ...
            <span class="col-8" v-html="mapping.s_description"> </span>
          </div>

          <div class="row">
            <span class="col-2 font-weight-bold">Creator:</span>
            <span class="col-8"> {{ mapping.s_contributor }}</span>
          </div>
          <div class="row">
            <span class="col-2 font-weight-bold">Publisher:</span>
            <span class="col-8"> {{ mapping.s_publisher }}</span>
          </div>
          <div class="row">
            <span class="col-2 font-weight-bold">Date:</span>
            <span class="col-8"> {{ mapping.s_publishedDate }}</span>
          </div>
        </b-tab>
        <b-tab id="web" title="Web Links" aria-labelledby="web-tab">
          <div class="row w-100">
            <span class="col-4 font-weight-bold">Name</span>
            <span class="col-8 font-weight-bold">link </span>
          </div>

          <div v-if="mapping.s_url" class="row">
            <span class="col-4">Object URL</span>
            <a class="col-8" :href="mapping.s_url" target="_blank">
              {{ mapping.s_url }}
            </a>
          </div>

          <div v-for="i in mapping.s_downloads" :key="i.name" class="row">
            <span class="col-4">{{ i.name }}</span>
            <a class="col-8" target="_blank" :href="i.contentUrl">{{
              i.contentUrl
            }}</a>
          </div>
        </b-tab>
        <b-tab id="cite" title="Citation" aria-labelledby="cite-tab">
          <div class="row">
            <span class="col-4 font-weight-bold">Citation</span>

            <a class="col-8" :href="mapping.s_citation" target="_blank">{{
              mapping.s_citation
            }}</a>
          </div>
        </b-tab>
        <b-tab id="json" title="Tool JSON-LD" aria-labelledby="json-tab">
          <div class="row ml-2">
            <div>JSON</div>
...
export default {
  ...
  methods: {
    toMetadataTools() {
      ...
      console.info(
        `placename:${placename} box:${box} poly:${poly} points:${points}`
      );
