      ...
      >
        <div class="tool_info pr-3">
          <b-link
            ...
            @click.stop="
              $router.push({ name: 'dataset', params: { d: i.g.value } })
            "
          >
          ...
          <h6 class="tool_title text-primary" v-html="i.name.value"></h6>
          <div class="small">
        ...
        <div v-if="false" class="buttons mt-3">
          <b-button
            variant="outline-primary"
            @click.stop="toRelatedData(i.g.value)"
            >Open</b-button
          >
        </div>
      </div>
    ...
    async showRelatedData() {
      ...
      try {
        ...
        let realtedTextFields =
          schemaItem("keywords", jp) +
          schemaItem("name", jp) +
          schemaItem("description", jp);
        ...
        console.error(
          "Failed to load related data query or process data:",
          error
        );
      } finally {
        this.running = false;
